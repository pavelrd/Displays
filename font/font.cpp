...
/**
 *
 * @brief Создает новый шрифт
 *
 * @param _size размер шрифта
 *
 */

Font::Font( uint8_t _size )
{
    setSize(_size);
}
...
void Font::setSize( uint8_t _size )
{
    size = _size;
}
...
/**
 *
 * @brief Получить текущий размер шрифта
 *
 * @return размер шрифта 1, 2, 3...
 *
 */

uint8_t Font::getSize()
{
   return size;
}
...
/**
    ...
    @brief Получить указатель на символ по его utf8 коду
	
    @param utf8Code код символа
	
    @return указатель на символ состоящий из SYMBOL_SIZE_IN_BYTES, если не найден то возвращает указатель на символ пробела
	
*/

const uint8_t* Font::getSymbolFromUtf8Code( uint16_t utf8Code )
{
	
	uint16_t offsetInFontTable;
	
    if( ( utf8Code >= SPACE_SYMBOL_CODE) && ( utf8Code <= DELETE_SYMBOL_CODE ) )
    {
		
		offsetInFontTable = utf8Code - SPACE_SYMBOL_CODE + UTF8_START_ASCII_OFFSET;

    }
    else if( ( utf8Code >= CYRILLIC_CAPITAL_LETTER_A ) && ( utf8Code <= CYRILLIC_CAPITAL_LETTER_YA ) )
    {
		
		offsetInFontTable = utf8Code - CYRILLIC_CAPITAL_LETTER_A + CYRILLIC_UPPER_CASE_OFFSET;
	
    }
    else if( ( utf8Code >= CYRILLIC_SMALL_LETTER_A ) && ( utf8Code <= CYRILLIC_SMALL_LETTER_PE ) )
    {
				
		offsetInFontTable = utf8Code - CYRILLIC_SMALL_LETTER_A + CYRILLIC_LOWER_CASE_OFFSET;

    }
    else if ( ( utf8Code >= CYRILLIC_SMALL_LETTER_ER ) && ( utf8Code <= CYRILLIC_SMALL_LETTER_YA) )
    {
		
		offsetInFontTable = utf8Code - CYRILLIC_SMALL_LETTER_ER + CYRILLIC_SMALL_ER_OFFSET;
		
    }
    else if( utf8Code == CYRILLIC_CAPITAL_LETTER_IO )
    {
	
		offsetInFontTable = CYRILLIC_CAPITAL_IO_OFFSET;
		
    }
    else if( utf8Code == CYRILLIC_SMALL_LETTER_IO )
    {
		
		offsetInFontTable = CYRILLIC_SMALL_IO_OFFSET;
			
    }
    else
    {
	
		offsetInFontTable = UNKNOWN_SYMBOL_OFFSET;
		
	}
	
	return &( font[ offsetInFontTable ][0] );

}


/**
 * @brief getSymbolWidth
 * @return
 */

uint8_t Font::getSymbolWidth()
{
    return WIDTH_IN_DOTS;
}

/**
 * @brief getSymbolHeight
 * @return
 */

uint8_t Font::getSymbolHeight()
{
    return HEIGHT_IN_DOTS;
}

/**
 * @brief getHorizontalSpacing
 * @return
 */

uint8_t Font::getHorizontalSpacing()
{
    return HORIZONTAL_SPACING;
}

/**
 * @brief getVerticalSpacing
 * @return
 */

uint8_t Font::getVerticalSpacing()
{
...
const uint8_t Font::font[FONT_SYMBOLS_COUNT][SYMBOL_SIZE_IN_BYTES] =
{
